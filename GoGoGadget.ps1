...
[scriptblock]${__GoGoGadgetCompleterBlock} = {
    param(
            $WordToComplete,
            $CommandAst,
            $CursorPosition
    ...
    $Values | ForEach-Object {
        ...
        switch ($Mode) {
            ...
            Default {
                ...
                $CompletionText = $($comp.Name | __GoGoGadget_escapeStringWithSpecialChars)
                if ($ExecutionContext.SessionState.LanguageMode -eq "FullLanguage"){
                    [System.Management.Automation.CompletionResult]::new($CompletionText, "$($comp.Name)", 'ParameterValue', "$($comp.Description)")
                } else {
                    $CompletionText
...
Register-ArgumentCompleter -CommandName 'GoGoGadget' -ScriptBlock ${__GoGoGadgetCompleterBlock}
